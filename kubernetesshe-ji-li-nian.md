### Kubernetes设计理念与分布式系统 {#kubernetes设计理念与分布式系统}
...
## Kubernetes的核心技术概念和API对象 {#kubernetes的核心技术概念和api对象}
...
### 复制控制器（Replication Controller，RC） {#复制控制器（replication-controller，rc）}
...
### 后台支撑服务集（DaemonSet） {#后台支撑服务集（daemonset）}
...
### 持久存储卷（Persistent Volume，PV）和持久存储卷声明（Persistent Volume Claim，PVC） {#持久存储卷（persistent-volume，pv）和持久存储卷声明（persistent-volume-claim，pvc）}
...
### 用户帐户（User Account）和服务帐户（Service Account） {#用户帐户（user-account）和服务帐户（service-account）}
...
参考：https://feisky.gitbooks.io/kubernetes/architecture/concepts.html
